...
export function ProviderSelection({
  ...
  // Group quotes by provider and get the best quote for each provider
  const providerQuotes = quotes.reduce((acc, quote) => {
    const provider = quote.serviceProvider;
    if (!acc[provider] || quote.totalFee < acc[provider].totalFee) {
      acc[provider] = quote;
    }
    return acc;
      ...
      <div className="max-h-96 overflow-y-auto">
        {filteredProviders.length > 0 ? (
          <div className="space-y-2">
            {filteredProviders
              .sort((a, b) => providerQuotes[a].totalFee - providerQuotes[b].totalFee) // Sort by lowest fees first
              .map((provider) => (
                <ProviderCard
                  key={provider}
                  serviceProvider={provider}
                  quote={providerQuotes[provider]}
